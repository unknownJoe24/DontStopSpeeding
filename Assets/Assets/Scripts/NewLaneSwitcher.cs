...
public class NewLaneSwitcher : MonoBehaviour
{

    public GameObject transformNodes; //this is the group which contains (left, current, middle)
    public Transform leftNode, middleNode, rightNode; 
    ...
    private int _leftLane = 1;
    private int _middleLane = 0;
    private int _rightLane = -1; 
    ...
    {
        currentLane = 0;
        gameObject.transform.position = middleNode.position; 
        rb = GetComponent<Rigidbody>();
        rb.velocity = new Vector3(0f, 0f, moveSpeed);       // Set initial movement velocity of RB
        parentZ = gameObject.transform.position.z;
        transformNodes.transform.position = new Vector3(transformNodes.transform.position.x, transformNodes.transform.position.y, parentZ);
    }
    ...
    void Update()
    {
        ...
        {
            targetLane = -1;
            targetPosition = rightNode; 
        }

        if ((Input.GetKeyDown(KeyCode.D) && currentLane == 1) || (Input.GetKeyDown(KeyCode.A) && currentLane == -1))
        {
            targetLane = 0;
            targetPosition = middleNode;
        }
        // If input is negative, move to the left lane
        else if (Input.GetKeyDown(KeyCode.A) && currentLane == 0)
        {
            targetLane = 1;
            targetPosition = leftNode;
        }
    ...
    private void FixedUpdate()
    {
        ...
        gameObject.transform.position = Vector3.MoveTowards(transform.position, targetPosition.position, Time.deltaTime * laneChangeSpeed);
